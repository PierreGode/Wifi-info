...
MATCHINGMODS="(air|ar5|at7|ath[^3]?|b43|bcma|brcm|carl|ipw|iwl|ndis|r(818|8192[eu]|871|92su)|8(188|189|192|723|812)[acde][esu]|rt[23567]|rtl|ssb|wl|(cfg|mac)80211)"
LSMATCHINGMODS="(wmi|(dell|ideapad)[-_]laptop)"
...
printf "\n##### lsmod #############################\n\n"
LSMOD=$(lsmod | egrep "(^|[[:punct:] ])($MATCHINGMODS|$LSMATCHINGMODS)[^[:punct:] ]*([[:punct:] ]|$)")
echo "$LSMOD"
...
printf "\n##### module infos ######################\n\n"
MODULES=$(egrep -o "^$MATCHINGMODS[^ ]*" <<< "$LSMOD")
...
printf "\n##### dmesg #############################\n\n"
dmesg | tail -n 100 | egrep "[[:punct:] ]($MATCHINGMODS|$IFACEMATCHES|$DMESGMATCHES)[^[:punct:] ]*[[:punct:] ]" | egrep -v "$DMESGEXCL" | uniq -cf 2 | sed 's/^[ ]\+1[ ]\+//;s/^[ ]\+\([0-9]\+\)[ ]\+\(.\+\)$/\2 (repeated \1 times)/'
